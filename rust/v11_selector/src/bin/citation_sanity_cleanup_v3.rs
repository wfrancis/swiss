...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 10 {
        eprintln!(
            ...
            args.first()
                .map(String::as_str)
                .unwrap_or("citation_sanity_cleanup_v3")
        );
        std::process::exit(2);
    }
    ...
    let summary = Summary {
        rules: vec![
            ...
            "remove any citation absent from laws_de, court_considerations, train gold, and val gold".to_string(),
        ],
...
fn clean_split(
    ...
    for result in rdr.deserialize() {
        ...
        for citation in before.iter() {
            ...
            } else if !law_universe.contains(citation)
                && !court_universe.contains(citation)
                && !gold_universe.contains(citation)
            {
                row_removals.push((
                    citation.clone(),
                    "citation absent from exact citation universe".to_string(),
                ));
            } else {
