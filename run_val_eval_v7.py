"""
V7 pipeline: GPT-5.4 centric with smart cutoff + fuzzy matching + live reranking.
Key changes from V6:
1. Fuzzy matching for GPT predictions that don't exactly match corpus
2. Smart cutoff using estimated_citation_count instead of always 40
3. Score recalibration (corpus-verified GPT = highest)
4. Optional live GPT-5.4 candidate reranking
"""
import csv
import gc
import json
...
# Optional: enable live GPT reranking (costs ~$0.03/query)
ENABLE_LIVE_RERANK = os.getenv("V7_LIVE_RERANK", "0") == "1"
...
def main():
    ...
    for row in val_rows:
        ...
        # === 12. SMART CUTOFF (key V7 improvement) ===
        # Use GPT estimated count as a HARD guide
        gpt_estimate = exp.get("estimated_citation_count", 25)

        # Count high-confidence predictions (these always get included)
        high_conf_count = sum(1 for c, s in verified if s >= 0.80)

        # Target: use GPT estimate as primary guide
        # Only exceed it significantly if we have lots of high-confidence predictions
        target = gpt_estimate
        if high_conf_count > target:
            target = high_conf_count  # Include all high-confidence
        target = max(target, 10)
        target = min(target, gpt_estimate + 8)  # Never exceed estimate by more than 8
        target = min(target, 40)  # Hard cap
